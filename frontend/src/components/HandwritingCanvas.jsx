...
export default function HandwritingCanvas({ 
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-2.5 rounded-xl bg-[#0a1329]/95 border border-blue-500/25 shadow-lg">
        {/* Tool Selectors */}
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => setTool('pen')}
            className={`p-2 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-all ${
              tool === 'pen' ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/30 font-semibold' : 'text-slate-300 hover:text-white hover:bg-blue-950/60'
            }`}
          ...
          <button
            ...
            className={`p-2 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-all ${
              tool === 'eraser' ? 'bg-slate-700 text-white font-semibold' : 'text-slate-300 hover:text-white hover:bg-blue-950/60'
            }`}
            title="Eraser"
          >
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-5 h-5 rounded-full border transition-transform ${
                  strokeColor === c.hex && tool === 'pen' ? 'scale-125 border-white ring-2 ring-blue-400' : 'border-slate-600 hover:scale-110'
                }`}
            ...
            {[2, 4, 7].map((size) => (
              <button
                ...
                className={`px-2 py-1 rounded transition-colors ${
                  strokeWidth === size ? 'bg-blue-600 text-white font-bold' : 'hover:bg-blue-950/60 text-slate-400 hover:text-slate-200'
                }`}
              >
                {size}px
              </button>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 text-slate-400 hover:text-slate-200 disabled:opacity-30 rounded hover:bg-blue-950/60 transition-colors"
            title="Undo"
          >
          ...
          <button
            onClick={handleClear}
            className="text-xs text-slate-400 hover:text-rose-400 px-2.5 py-1.5 rounded hover:bg-rose-950/30 transition-colors"
          >
            Clear
          </button>

          <label className="text-xs text-slate-200 hover:text-white px-3 py-1.5 rounded-lg bg-[#0f1d42] hover:bg-[#152654] border border-blue-500/30 shadow-xs cursor-pointer flex items-center gap-1.5 font-medium transition-colors">
